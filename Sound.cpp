...
void Sound::Play(SOUND_LABEL label)
{
	...
	if (pSV != nullptr) // 复用现有的 SourceVoice
	{
		pSV->Stop(0);  // 停止当前播放
		pSV->FlushSourceBuffers(); // 清空缓冲
	}
	else // 只有在没有创建 SourceVoice 时才创建
	{
		HRESULT hr = m_pXAudio2->CreateSourceVoice(&pSV, &(m_wfx[(int)label].Format));
		if (FAILED(hr))
		{
			pSV = nullptr; // 避免访问无效指针
			return;
		}
	}
